...
class VolumeRenderer(object):
    ...
    def forward_rendering(self, H, output, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles):
        ...
        if 'ref_vertices' in H:
            ...
            with torch.no_grad():
                ...
                for iter_num in range(((resol*resol*resol)//14336) - 1):
                    ...
                    distances = torch.cdist(ref_vertices, p_i[..., 1:])
            ...
            result = result.reshape(resol,resol,resol).numpy()
            print("mrcfile")
            import mrcfile
            result_0 = np.flip(result, 0)
            with mrcfile.new_mmap(f'density_mrc_{resol}_shape_ray_deform.mrc', overwrite=True, shape=result.shape, mrc_mode=2) as mrc:
                mrc.data[:] = result_0
        ...
        ## original vertex와 mathching
        # displacement_v = original_v - shited_v
        
        ## shifted vertex displacement를 matching된 point별로 적용
        # NN btw original_v and p_i
