...
class VolumeRenderer(object):
    ...
    def forward_rendering(self, H, output, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles):
        ...
        if "test_vertex" in H:
            resol = 224
            #p_x = np.linspace(-0.122,0.122,resol)
            p_x = np.linspace(-0.122,0.122, resol)
            
            p_all = np.zeros((1, resol*resol*resol,3)).astype(np.float32)
            count = 0
            
            for x in range(len(p_x)):
                for y in range(len(p_x)):
                    for z in range(len(p_x)):
                        p_all[0, count, 0] = p_x[x]
                        p_all[0, count, 1] = p_x[y]
                        p_all[0, count, 2] = p_x[z]
                        count += 1
                       
            result = torch.zeros((1, resol*resol*resol,1)).float()
            result_2 = torch.ones((1, resol*resol*resol,1)).float() * -100.0
            # per sampeling 14336.to(H.device)
            #import ipdb; ipdb.set_trace()
            with torch.no_grad():
                print(resol*resol*resol//14336)
                for iter_num in range(((resol*resol*resol)//14336) - 1):
                    print(iter_num)
                    p_i = torch.from_numpy(p_all[:, iter_num* 14336: (iter_num +1)*14336, :]).float().to(H.device)
                    if nerf_input_feats is not None:
                        p_i = self.I.query_input_features(p_i, nerf_input_feats, fg_shape, bound)        
                    
                    feat, sigma_raw = fg_nerf(p_i, r_i, z_shape_obj, z_app_obj, ws=styles, shape=fg_shape)
                    result[:, iter_num* 14336: (iter_num +1)*14336, :] = sigma_raw.cpu()
                    

            
            result = result.reshape(224,224,224).numpy()
            result_2 = result_2.reshape(224,224,224).numpy()
            # import ipdb; ipdb.set_trace()
            def scale_coordinates(coords, in_range, out_range):
                ...
                return scaled_coords.cpu().numpy().astype(int)
            
            ### -0.112 0.112 에서 512/나눠서 몇번쨰 인덱스 값인지 넣어줄 수 있음
            ### 좌표계 비율이 아니라 linspace나눈 기준으로!
            bin = p_x[1] - p_x[0]
            
            ##
            #scaled_coords = scale_coordinates(H['test_vertex_scaled'], (-0.122, 0.122), (0, 224))
            scaled_coords = H['test_vertex_scaled'].cpu().numpy()
            for idx in range(35709):
                
                x_temp = int(scaled_coords[0, idx, 0]/ bin + 112)
                y_temp = int(scaled_coords[0, idx, 1]/ bin + 112)
                #import ipdb; ipdb.set_trace()
                #result_2[:, 224-x_temp , y_temp] = result[:, 224-x_temp, y_temp]
                result_2[:, x_temp , y_temp] = result[:, x_temp, y_temp]
            
            #import ipdb; ipdb.set_trace()
            #print("marching_cubes")
            #import mcubes
            #result_sorted= [x for x in result.tolist() if x[-1] != 1000]
            #result_sorted = np.array(result_sorted) 
            #vertices, triangles = mcubes.marching_cubes(result, 0)
            #mcubes.export_obj(vertices, triangles, f'density_{resol}_new.obj')
            
            print("mrcfile")
            import mrcfile
            result_0 = np.flip(result, 0)
            with mrcfile.new_mmap(f'density_mrc_{resol}_shape_3dmm_render_all.mrc', overwrite=True, shape=result.shape, mrc_mode=2) as mrc:
                mrc.data[:] = result_0
            
            #print("mrcfile")
            #import mrcfile
            result_0 = np.flip(result_2, 0) #np.flip(result_2, 0)
            with mrcfile.new_mmap(f'density_mrc_{resol}_shape_3dmm_render_reScaled_ver2_224.mrc', overwrite=True, shape=result.shape, mrc_mode=2) as mrc:
                mrc.data[:] = result_0
