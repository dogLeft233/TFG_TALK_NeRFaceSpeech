...
class VolumeRenderer(object):
    ...
    def forward_rendering(self, H, output, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles):
        ...
        if 'ref_vertices' in H:
            
            ref_vertices = H['ref_vertices']
            pred_vertices = H['pred_vertices']
            displacement = ref_vertices - pred_vertices
            
            #displacement[:, :, 0] = 0.
            #print("here")
            #import ipdb; ipdb.set_trace()
            
            if "nearest_indices" in H:
                nearest_indices = H['nearest_indices']
                output.nearest_indices = nearest_indices
            else:
                distances = torch.cdist(ref_vertices, p_i[..., 1:]) # 왜 1부터..? z 제외한거임
                nearest_indices = torch.argmin(distances, dim=1) # tensor([[28429, 28429, 28429,  ..., 34913, 34913, 34913]], device='cuda:0')
                output.nearest_indices = nearest_indices
                
                
            '''
            for p_index in range(p_i.size(1)):
                p_nearest_idx = nearest_indices[0][p_index]
                p_i[:, p_index, 1:] = p_i[:, p_index, 1:] + displacement[:, p_nearest_idx, :]
            '''

            p_nearest_idx = nearest_indices[0]
            p_i[:, :, 1:] = p_i[:, :, 1:] + displacement[:, p_nearest_idx, :]
        ...
        if 'sigma_raw_coarse' in H:
            sigma_raw = H['sigma_raw_coarse']
            feat = H['feat_coarse'] 
        else:
            output.feat_coarse = feat      
            output.sigma_raw_coarse = sigma_raw          
        ...
        if self.hierarchical and (not H.get('disable_hierarchical', False)):
            ...
            if 'ref_vertices' in H:
                ...
                displacement = ref_vertices - pred_vertices
                #displacement[:, :, 0] = 0.
                if "nearest_indices_f" in H:
                    nearest_indices = H['nearest_indices_f']
                    output.nearest_indices_f = nearest_indices
                else:
                    distances = torch.cdist(ref_vertices, p_f[..., 1:])
                    ##한번만 해도 됨 ref points와 ref_3dmm과의 관계임
                    nearest_indices = torch.argmin(distances, dim=1) # tensor([[28429, 28429, 28429,  ..., 34913, 34913, 34913]], device='cuda:0')
                    output.nearest_indices_f = nearest_indices
                    
                # 이건 매번 해야함 
                '''for p_index in range(p_f.size(1)):
                    p_nearest_idx = nearest_indices[0][p_index]
                    p_f[:, p_index, 1:] = p_f[:, p_index, 1:] + displacement[:, p_nearest_idx, :]'''
                
                p_nearest_idx = nearest_indices[0]
                p_f[:, :, 1:] = p_f[:, :, 1:] + displacement[:, p_nearest_idx, :]
            ...
            if 'sigma_raw_fine' in H:
                sigma_raw_f = H['sigma_raw_fine']
                feat_f = H['feat_fine']
            else:
                output.sigma_raw_fine = sigma_raw_f
                output.feat_fine = feat_f
    ...
    def forward_rendering_background(self, H, output, bg_nerf, nerf_input_cams, latent_codes, styles_bg):
        ...
        if 'seg_decoder' in H:
            #import ipdb; ipdb.set_trace()
            feat_seg = H['seg_decoder'](feat)
            feat = torch.cat([feat, feat_seg], -1)
...
class NeRFSynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, **block_kwargs):
        ...
        with torch.autograd.profiler.record_function('nerf'):
            ...
            if "inner_mouth_feat" in outputs:
                block_kwargs['inner_mouth_p_i'] = outputs.inner_mouth_p_i
                block_kwargs['inner_mouth_feat'] = outputs.inner_mouth_feat
                block_kwargs['inner_mouth_sigma_raw'] = outputs.inner_mouth_sigma_raw
                
            if "inner_mouth_feat_f" in outputs:
                block_kwargs['inner_mouth_p_f'] = outputs.inner_mouth_p_f  
                block_kwargs['inner_mouth_feat_f'] = outputs.inner_mouth_feat_f
                block_kwargs['inner_mouth_sigma_raw_f'] = outputs.inner_mouth_sigma_raw_f
            
            if 'di_fine' in outputs:
                block_kwargs['di'] = outputs.di
                block_kwargs['di_fine'] = outputs.di_fine
                
            if 'p_f' in outputs:
                block_kwargs['p_i'] = outputs.p_i
                block_kwargs['p_f'] = outputs.p_f
            
            if 'sigma_raw_coarse' in outputs:
                block_kwargs['sigma_raw_coarse'] = outputs.sigma_raw_coarse 
                block_kwargs['sigma_raw_coarse'] = outputs.sigma_raw_coarse  
                block_kwargs['feat_coarse'] = outputs.feat_coarse 
                block_kwargs['sigma_raw_fine'] = outputs.sigma_raw_fine 
                block_kwargs['feat_fine'] = outputs.feat_fine
            
            if "nearest_indices_f" in outputs:
                block_kwargs['nearest_indices'] = outputs.nearest_indices
                block_kwargs['nearest_indices_f'] = outputs.nearest_indices_f 
                
            reg_loss = outputs.get('reg_loss', {})
            x, img, _ = self.V.post_process_outputs(outputs['full_out'], self.freeze_nerf)
            
            if 'seg_decoder' in block_kwargs:
                x_seg = x[:,256:,:,:] # torch.Size([B, 16(class), 32, 32])
                block_kwargs['seg'] = x_seg
                x = x[:,:256,:,:]
        ...
        if 'feat_inpaint_network_bicubic' in block_kwargs:
            x = block_kwargs['feat_inpaint_network_bicubic'](x, block_kwargs['pred_mask_proc_32'], size=512, Maskmode="bicubic", Featmode="bicubic")
            
        #gihoon
        if 'inpaint_feat' in block_kwargs:
            if 'inpaint_mask' in block_kwargs:
                inpaint_mask  = block_kwargs['inpaint_mask']
                inpaint_mask = torch.nn.functional.interpolate(inpaint_mask, size=(block_kwargs['interpol_size'],block_kwargs['interpol_size']), mode=block_kwargs['Mask_method'])
                   
                x = inpaint_mask * torch.nn.functional.interpolate(x, size=(block_kwargs['interpol_size'],block_kwargs['interpol_size']), mode=block_kwargs['Feat_method']) + \
                    (1. - inpaint_mask) * torch.nn.functional.interpolate(block_kwargs['inpaint_feat'], size=(block_kwargs['interpol_size'],block_kwargs['interpol_size']), mode=block_kwargs['Feat_method'])
                    
                x = torch.nn.functional.interpolate(x, size=(32,32), mode=block_kwargs['Feat_method'])
            else:
                x = block_kwargs['inpaint_feat']
        else:
            block_kwargs['inpaint_feat'] = x
            
            
        if 'eye_inpaint_feat' in block_kwargs:
            #print("here")
            eye_inpaint_mask  = block_kwargs['eye_inpaint_mask']
            eye_inpaint_mask = torch.nn.functional.interpolate(eye_inpaint_mask, size=(block_kwargs['eye_interpol_size'],block_kwargs['eye_interpol_size']), mode=block_kwargs['eye_Mask_method'])
            
            x = eye_inpaint_mask * torch.nn.functional.interpolate(x, size=(block_kwargs['eye_interpol_size'],block_kwargs['eye_interpol_size']), mode=block_kwargs['eye_Feat_method']) + \
                (1. - eye_inpaint_mask) * torch.nn.functional.interpolate(block_kwargs['eye_inpaint_feat'], size=(block_kwargs['eye_interpol_size'],block_kwargs['eye_interpol_size']), mode=block_kwargs['eye_Feat_method'])
                
            x = torch.nn.functional.interpolate(x, size=(32,32), mode=block_kwargs['eye_Feat_method'])
